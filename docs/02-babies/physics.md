...
2. We set the `Connected Anchor` at `(0,1)` (this is local position, or in other words: the position of the anchor relative to the connected Rigidbody). That means to set the connected anchor to the connected Rigidbody in `Question-Box-Bounce` at 1 unit away in the y-axis direction from this GameObject as shown on the Scene. 
