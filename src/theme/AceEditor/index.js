...
function PyEditor({
  ...
  const [hasEdits, setHasEdits] = React.useState(
    getItem(codeId, contextId, {}).edited ? !slim : false
  );
  const [pyScript, setPyScript] = React.useState(
    hasEdits ? getItem(codeId, contextId, {}).edited : ""
  );
  ...
  const clearResult = (force = false) => {
    ...
    document.dispatchEvent(
      new CustomEvent(CLOSE_TURTLE_MODAL_EVENT, {
  ...
  const execScript = () => {
    ...
    const active = document.getElementById(
      DOM_ELEMENT_IDS.scriptSource(codeId)
    );
  ...
  const onReset = () => {
    ...
    const shouldReset = window.confirm(
      "Discard changes? (Your version will be lost!)"
    );
    if (shouldReset) {
      setShowRaw(true);
  ...
  return (
    <React.Fragment>
      ...
      <div className={clsx(styles.result)}>
        <Result logMessages={logMessages} codeId={codeId} />
        {turtleModalOpen && (
          <TurtleResult
            codeId={codeId}
...
export default function PyAceEditor({ codeId, slim, ...props }) {
  ...
  const onTurtleModalClose = React.useCallback((event) => {
    if (event.detail.force || event.detail.codeId !== codeId) {
      ...
      const resNode = document.getElementById(
        DOM_ELEMENT_IDS.turtleResult(codeId)
      );
